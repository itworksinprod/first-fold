...
export async function diagnosePreservationReview({ publicKey, accountId, apiToken, now,
  ...
  for (const control of PRESERVATION_REVIEW_CONTROLS) {
    const view = buildClaimwiseFactReview(control.input);
    // Byte-for-byte prompt assembly used by the existing fact-summary reviewer.
    ...
    try {
      ...
      const checked = validateClaimwiseFactReview(call.response, view);
  ...
  const report = { mode: 'preservation-review-controls-not-an-edition',
