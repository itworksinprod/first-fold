...
export function renderHumanReview(draft, dossier) {
...
<header><strong>UNAPPROVED — HUMAN REVIEW REQUIRED</strong><br>Offline writing sample from stored September 14, 2026 MIT evidence. Not today's paper, fresh research, or an approved edition. Flash-Lite failed automatic reviewer qualification; no email has been sent.</header>
...
<h2>Evidence provided to the writer</h2><p>These are stored excerpts, not a new check of the full article. <a href="https://news.mit.edu/2026/new-method-enables-ai-safety-critical-situations-0914" rel="noreferrer">Original MIT News article</a></p>
...
export async function previewGeminiLite({ apiKey, freeProjectConfirmation, fetchImpl = globalThis.fetch } = {}) {
  const dossier = reviewerResearchScopeCases()[0].dossier;
  ...
  try {
    ...
    html: renderHumanReview(result.editorialPayload.stories[0], dossier) };
  } catch (error) {
