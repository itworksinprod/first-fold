...
1. Reads only the project's checked-in, 30-source manifest. Nineteen first-party or regulator feeds cover AI research and releases, developer tools, security advisories, cloud platforms, and competition policy. Eleven reviewed independent outlets—Ars Technica, The Verge, TechCrunch, BleepingComputer, WIRED, The Register, The Guardian, BBC News, NPR, SecurityWeek, and Dark Reading—supply a bounded corroboration pool representing ten controlling publisher identities; Ars Technica and WIRED share one identity.
...
- **Narrower corroboration.** The source pool is intentionally bounded and remains first-party-heavy despite its eleven reviewed independent outlets, which represent ten controlling publisher identities. A feed URL is context only, and two brands controlled by one organization still count as one publisher. If two distinct reviewed publishers do not supply factual article pages for an event, that desk stays quiet.
