...
| Research retry | One bounded research retry below three stories; shared search/article/assessment budgets; choose one intact snapshot and never merge attempts | None; one bounded feed snapshot | Governed by the separate paid workflow |
...
1. Reads only the project's checked-in, 48-source manifest. Thirty-five first-party, research-institution, or regulator feeds cover AI research and releases, developer tools, security advisories, cloud platforms, and competition policy. Thirteen reviewed independent outlets—Ars Technica, The Verge, TechCrunch, BleepingComputer, WIRED, The Register, The Guardian, BBC News, NPR, SecurityWeek, Dark Reading, MIT Technology Review, and IEEE Spectrum—supply a bounded corroboration pool representing twelve controlling publisher identities; Ars Technica and WIRED share one identity, while MIT News and MIT Technology Review intentionally share the MIT identity across the originating and independent pools.
   The expanded AI and Work desks include Google DeepMind, Ai2, MIT News, Berkeley AI Research, Meta Engineering, Amazon Science, JetBrains, GitHub Engineering, Slack Engineering, Chrome for Developers, Node.js, Postman, the Rust Blog, Netlify, Ollama, and OpenRouter. The same 1 MB per-feed, 10 MB aggregate download, and 320-item limits remain in force. Ollama and OpenRouter are originating accounts of their own services, not independent confirmation of third-party model claims. See the [September 12 source review](source-discovery-review-2026-09-12.md).
...
3. Deduplicates and ranks those items before the model call. Matching normalizes Unicode, product/version identifiers, entity aliases, action wording, explicit vulnerability IDs, and typed numeric anchors while vetoing conflicting entities, versions, amounts, legal roles, and claim polarity. Every item admitted to a multi-source event must match every other item, so a generic bridge cannot fuse two unrelated stories. Publisher aliases under the same controlling organization collapse to one reviewed publisher identity—for example, the Google family is one publisher, and GitHub and Microsoft are one publisher.
...
- **Narrower corroboration.** The source pool is intentionally bounded and remains first-party-heavy despite its thirteen reviewed independent outlets, which represent twelve controlling publisher identities. A feed URL is context only, and two brands controlled by one organization still count as one publisher. If two distinct reviewed publishers do not supply factual article pages for an event, that desk stays quiet.
...
pass selects fewer than three stories, it performs one bounded feed-only retry
