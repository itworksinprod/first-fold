...
Optional web discovery uses `TAVILY_API_KEY` for eight daily news searches and up to four follow-ups, cached across the research retry. Twelve advanced searches reserve at most 24 credits per run, or 744 for one run on each of 31 days. A dedicated First Fold key must have a provider-enforced monthly limit of **900 credits**; the adapter verifies a free Researcher account and disabled pay-as-you-go before searching. Missing configuration, quota exhaustion, or search failures leave the feed pipeline available. Search pages share the existing 24-article fetch budget and unchanged editorial checks. This is an optional integration, not a claim that account setup or a live search test has been completed. [Setup and no-email verification](docs/personal-delivery.md#5-optionally-enable-free-web-discovery)
