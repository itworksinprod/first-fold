...
- **5:05 AM — the private paper begins.** The owner-only job reads curated live feeds and a bounded shortlist of linked publisher articles. Optional Tavily web discovery can find articles outside those feeds, but only reviewed publishers' own dated pages can become evidence; search snippets never do. Up to four free-tier Cloudflare calls assess importance and usefulness, write concrete source-grounded news summaries, optionally revise locally rejected drafts once, and check the final summaries against their evidence. Local gates enforce source eligibility, freshness, repeat suppression, scoring weights, thresholds, exact citations, and delivery safety. Rejected synthesis retains a deterministic source-bound fallback, not a Mac-local model edition. The optional [Mac-local model preview](docs/local-model-preview.md) is a separate manual workflow. The account must remain on Workers Free to prevent paid overages; there is no OpenAI or paid fallback. The paper stays private, with at most one story per desk. See [personal delivery](docs/personal-delivery.md) for the full controls and the no-email quality tests.
...
No environment variables or paid services are needed to build, read, preview, or publish a hand-written edition. Automatic personal research uses curated live feeds and at most four Cloudflare Workers AI calls under the account's free allocation. It has no paid fallback: if optional synthesis is unavailable or invalid, the source-bound local writer still produces the paper. Keep the Cloudflare account on Workers Free and do not enable prepaid AI Gateway credits if the requirement is a hard zero-dollar automatic path.

Optional web discovery uses `TAVILY_API_KEY` for eight daily news searches and up to four follow-ups, cached across the research retry. Searches target the exact article hosts in the reviewed publisher registry, so their limited result slots favor publishers the paper can actually verify. This is a search constraint, not evidence: each article still passes local identity, publication-date, text, and quality checks. Twelve advanced searches reserve at most 24 credits per run, or 744 for one run on each of 31 days. A dedicated First Fold key must have a provider-enforced monthly limit of **900 credits**; the adapter verifies a free Researcher account, remaining free credits, and no PAYGO usage. Keep pay-as-you-go disabled. If the API reports an unset PAYGO limit, explicit operator verification of the disabled billing setting is required; an unset limit alone is not proof. Missing configuration, quota exhaustion, or search failures leave the feed pipeline available. Search pages share the existing 24-article fetch budget and unchanged editorial checks. Complete account setup and a live test before describing search as operational. [Setup and no-email verification](docs/personal-delivery.md#5-optionally-enable-free-web-discovery)

The reviewed catalog includes official Ollama and OpenRouter feeds for local-model and developer-tool developments. New sources require review; neither a model nor a search result can automatically approve a publisher. [September 12 source review](docs/source-discovery-review-2026-09-12.md)
...
The **Personal Morning Paper** is the only automatically researched lane. It uses the free evidence-grounded pipeline described above and sends one private HTML/plain-text email through Resend. Candidates remain ephemeral; the existing keyed repeat ledger, public-safe source-health report, and optional private feedback keep their retention and privacy boundaries. Required setup remains `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_AI_API_TOKEN`, `RESEND_API_KEY`, `PERSONAL_PAPER_EMAIL`, and the dispatcher—no OpenAI key. See [the delivery guide](docs/personal-delivery.md) for setup, free-plan requirements, and quality-check controls.
