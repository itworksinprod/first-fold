...
- **5:05 AM — the private paper begins.** The owner-only job reads curated live feeds and a bounded shortlist of linked publisher articles. Up to three free-tier Cloudflare calls assess importance and usefulness, write concrete source-grounded news summaries, and check those summaries against their evidence. Local gates enforce source eligibility, freshness, repeat suppression, scoring weights, thresholds, exact citations, and delivery safety. Rejected synthesis retains the local baseline. The account must remain on Workers Free to prevent paid overages; there is no OpenAI or paid fallback. The paper stays private, with at most one story per desk. See [personal delivery](docs/personal-delivery.md) for the full controls and the no-email quality test.
